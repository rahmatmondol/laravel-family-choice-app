...
<?php
$page = 'schools';
$title = __('site.Create Subscription');
?>
@section('title_page')
{{ $title }}
@endsection
...
<div class="content-wrapper">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item"><a
                href="{{ route($mainRoutePrefix.'.schools.subscriptions.index',['school' => $school->id]) }}">@lang('site.subscription')</a>
            </li>
  ...
  <section class="content">
    <form method="post" action="{{ route($mainRoutePrefix.'.schools.subscriptions.store',['school'=>$school->id])}}"
          ...
          <div class="card card-primary">
            <div class="card-body">
              ...
              <div class="form-group">
                <label for="inputType">@lang('site.subscription')</label>
                <select name="subscription_id" class="form-control" required>
                  <option value="">@lang('site.subscription') </option>
                  @foreach( $subscriptions as $value )
                  <option value="{{ $value->id}}" @if( old('subscription_id')==$value->id ) selected @endif>
                    {{ $value->title }}</option>
                  @endforeach
                </select>
              </div>
