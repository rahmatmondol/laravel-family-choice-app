...
@section('content')
    ...
    <!-- END: BreadCrumb -->
    <form method="post" action="{{ route($mainRoutePrefix . '.subscriptionTypes.update', $subscriptionType->id) }}"
                    ...
                    <div class="card-text h-full space-y-4">
                        ...
                        <div class="input-area">
                            ...
                            <select id="inputStatus" name="type" required class="form-control custom-select mt-2">
                                ...
                                @foreach (\App\Enums\SubscriptionTypes::cases() as $sub_type)
                                    <option value="{{ $sub_type->value }}" @selected(old('type', $subscriptionType->type) == $sub_type->value)>
                        ...
                        <div class="input-area">
                            ...
                            <select id="inputStatus" name="number_of_days" required class="form-control mt-2">
                                ...
                                @for ($i = 1; $i <= 7; $i++)
                                    <option value="{{ $i }}" @selected(old('number_of_days', $subscriptionType->number_of_days) == $i)>{{ $i }}
                                    </option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="input-area">
                            ...
                            <input type="text" name="order_column"
                                value="{{ old('order_column', $subscriptionType->order_column) }}"
                        ...
                        <div class="input-area">
                            ...
                            <select id="inputStatus" name="status" required class="form-control custom-select mt-2">
                                <option value='' selected disabled>@lang('site.Status')</option>
                                <option value="1" @if (old('status', $subscriptionType->status) == 1) selected @endif>@lang('site.Active')
                                </option>
